 ...
 */
@Component
public class Upload_GraChAnd42Day_42Day_Visit extends BaseTask implements TaskInterface {
	private static final Logger logger = Logger.getLogger(Upload_GraChAnd42Day_42Day_Visit.class);
	...
	public String getSQL(JdbcTemplate jdbcTemplate) {
		...
		logger.info("+++++++++++++++++++++++++++++++======SQL:");
		buffer.append(" SELECT 	hcall42.id as grahcall42id,	");
		...
		buffer.append(" hcall42.change_unit as cljg,	");// 转诊机构,是 String
		buffer.append(" hcall42.change_cause as clyy,	");// 转诊原因,是 String
		buffer.append(" DATE_FORMAT(tgl.leave_hospital_date,'%Y-%m-%d') as cyrq,	");// 出院日期,是 String
		buffer.append(" if(hcall42.el_taste='1' or hcall42.el_taste='' or hcall42.el_taste is NULL,'0','1') as elu,	");// 恶露：未见异常： 0；异常： 1,是 String
		buffer.append(" if(hcall42.el_taste='2','有臭味','无') as elyc,	");// 恶露异常,是 String
		buffer.append(" if(hcall42.classify='已恢复','0','1') as fenlei,	");// 分类：未见异常： 0；异常： 1,是 String
		buffer.append(" if(hcall42.classify='未恢复',hcall42.classify_other,'无') as flyc,	");// 分类异常,是 String
		buffer.append(" DATE_FORMAT(tgl.labor_date,'%Y-%m-%d') as fmsj,	");// 分娩时间,是 String
		buffer.append(" apv.id as grdabh,	");// 个人档案编号,是 String
		//buffer.append(" hcall42.id as id,	");// 主键 id,是 String
		buffer.append(" copy.yc_num as jkid,	");// 单条访视 id,是 String
		buffer.append(" apv.name as name,	");// 姓名,是 String
		buffer.append(" hcall42.other as qita,	");// 其他,是 String
		buffer.append(" if(hcall42.breast in('01','02'),'1','0') as rufang,	");// 乳 房：未见异常：0；异常： 1,是 String
		buffer.append(" case hcall42.breast when '01' then '乳腺炎' when '02' then '乳汁淤积' else '无' end as rfyc,	");// 乳 房异常信息,是 String
		buffer.append(" DATE_FORMAT(hcall42.check_date,'%Y-%m-%d') as sfrq,	");// 随访日期,是 String
		buffer.append(" hd.name as sfys,	");// 随访医生,是 String
		buffer.append(" hcall42.add_user_id as sfyscode,	");// 随访医生 id,是 String
		buffer.append(" if(hcall42.fb_incision in('1','6','7','8') or hcall42.fb_incision='' or hcall42.fb_incision is NULL,'0','1')  as shangkou,	");// 伤 口：未见异常：0；异常： 1,是 String
		...
		buffer.append(" '' as tiwen,	");// 体 温,是 String
		buffer.append(" '1' as wzd,	");// 完整度,是 String
		buffer.append(" '' as xcsfrq,	");// 下次随访日期,是 String
		buffer.append(" hcall42.up_blood as xy,	");// 血压,是 String
		buffer.append(" hcall42.low_blood as xy1,	");// 血压,是 String
		buffer.append(" if(hcall42.health='' or hcall42.health is NULL,'未见异常',hcall42.health) as ybjkzk,	");// 一般健康情况,是 String
		buffer.append(" if(hcall42.mental='' or hcall42.mental is NULL,'未见异常',hcall42.mental) as ybxlzk,	");// 一般心理状况,是 String
		buffer.append(" '无' as zdqt,	");// 指导其他,是 String
		buffer.append(" case hcall42.uterine_neck when '2' then '外口粘合' when '3' then '宫颈瘢痕' when '4' then '宫颈水肿'	");
		buffer.append(" when '5' then '宫颈坚韧' when '6' then '宫颈肌瘤' when '7' then '其他' when '8' then '充血' else '无' end as zgyc,	");// 子宫异常,是 String
		buffer.append(" concat(if(hcall42.guide_type like '%性保健与避孕%','1','0'),',',if(hcall42.guide_type like '%心理保健%','1','0'),',',if(hcall42.guide_type like '%产妇营养%','1','0'),',',	");
		buffer.append(" if(hcall42.guide_type like '%纯母乳喂养%','1','0'),',',if(hcall42.guide_type like '%婴儿喂养%','1','0'),',','0') as zhidao,	");// 多选指导:个人卫生，心理，营养，母乳喂养，新生儿护理与喂养，其他(zzqt),是 String
		buffer.append(" if(hcall42.changer='是','1','0') as zhuanzhen,	");// 转诊：无： 0，有： 1,是 String
		buffer.append(" if(hcall42.uterine_neck in('2','3','4','5','6','7','8'),'1','0') as zigong,	");// 子 宫：未见异常：0；异常： 1,是 String
		...
		buffer.append(" hcall42.add_user_id as czy,	");// 录入人 id,是 String
		buffer.append(" DATE_FORMAT(hcall42.add_time,'%Y-%m-%d') as czrq,	");// 录入日期,是 String
		buffer.append(" '1' as sfbz,	");// 0:产后访视， 1:42 天检查,是 String
		buffer.append(" DATE_FORMAT(hcall42.add_time,'%Y-%m-%d') as createdate,	");// 录入日期,是 String
		buffer.append(" hcall42.update_user_id as updateid,	");// 修改人 ID,是 String
		buffer.append(" if(hcall42.update_time is NULL,DATE_FORMAT(hcall42.add_time,'%Y-%m-%d'),DATE_FORMAT(hcall42.update_time,'%Y-%m-%d')) as updatetime	");// 修改日期,是 String
		buffer.append(" FROM t_gra_labor42 hcall42	");
		buffer.append(" JOIN t_gra_person AS gp ON gp.id=hcall42.person_id and gp.statue='0'	");
		buffer.append(" JOIN t_arc_person AS apv ON apv.id=gp.base_info_id and apv.statue='0'	");
		buffer.append(" JOIN t_gra_labor AS tgl ON tgl.person_id=hcall42.person_id and tgl.statue='0'	");
		buffer.append(" JOIN t_hos_doctor AS hd ON hd.user_id=hcall42.add_user_id	");
		buffer.append(" JOIN t_hos_hospital AS hh ON hh.id=hd.hospital_id LEFT JOIN t_gra_build_copy copy on gp.id=copy.person_id ");
		buffer.append(" WHERE hcall42.statue='0' ");// and apv.card_id='370829199802031127'
		buffer.append(" and hcall42.check_date >= ").append(date1);
		buffer.append(" and hcall42.check_date <= ").append(date2);
	...
	public Map<String,Object> getStateSQL(Map<String, Object> objs) {
		...
		String grahcall42id = AddressUtil.toStr(objs.get("grahcall42id"));//业务id
		String jsondata;
		...
		try {
			...
			resuletMap.put("grahcall42id",grahcall42id);
	...
	public void logAfter(JdbcTemplate jdbcTemplate,Map<String, Object> objs) {
		...
		String grahcall42id = (String) objs.get("grahcall42id");
		...
		bf.append("'").append(grahcall42id).append("' and upload_type='妇女42天记录(t_gra_labor42)'");
		...
		buffer.append("'").append(grahcall42id).append("',");// grahcall42id
		...
		buffer.append("'").append("妇女42天记录(t_gra_labor42)").append("'");
		buffer.append(")"); //
	...
	public static void main(String[] args) {
		//System.out.println(new Upload_GraChAnd42Day_42Day_Visit().getSQL());
	}
